...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['acao'] ?? '') === 'criar') {
  ...
  if ($titulo !== '' && $descricao !== '') {
    $st = $pdo->prepare("INSERT INTO sobre (titulo, descricao, foto_path) VALUES (:t,:d,:f)");
    $st->execute([':t'=>$titulo, ':d'=>$descricao, ':f'=>$foto]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['acao'] ?? '') === 'atualizar') {
  ...
  if ($titulo !== '' && $descricao !== '') {
    if ($fotoNova) {
      ...
      $sql = "UPDATE sobre SET titulo=:t, descricao=:d, foto_path=:f WHERE id=:id";
      $args = [':t'=>$titulo, ':d'=>$descricao, ':f'=>$fotoNova, ':id'=>$id];
    } else {
      $sql = "UPDATE sobre SET titulo=:t, descricao=:d WHERE id=:id";
      $args = [':t'=>$titulo, ':d'=>$descricao, ':id'=>$id];
    }
...
<form method="post" enctype="multipart/form-data">
  ...
  <p>Descrição:<br><textarea name="descricao" rows="6" cols="60" required></textarea></p>
...
<form method="post" enctype="multipart/form-data">
  ...
  <p>Descrição:<br><textarea name="descricao" rows="6" cols="60" required><?= e($edit['descricao']) ?></textarea></p>
